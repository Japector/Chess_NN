...
from utils.utility import board_to_multichannel_array, generate_move_maps
...
def select_move_epsilon_greedy(board, model, move_to_int, epsilon=EPSILON):
    ...
    q_values = model.predict(board_to_multichannel_array(board).reshape(1, 8, 8, 12).astype(np.float32), verbose=0)[0] 
    best_move = max(legal_moves, key=lambda m: q_values[move_to_int.get(m.uci(), 0)]) 
    return best_move


# ==== REWARD FUNCTION ====
def calculate_reward(board, move, engine=None):
...
def collect_experience(board, model, move_to_int, stockfish = False):
    state_array = board_to_multichannel_array(board).astype(np.float32)
    ...
    next_state_array = board_to_multichannel_array(board).astype(np.float32)
...
def train_from_replay(model):
    ...
    history = model.fit(states, targets, epochs=1, verbose=0) # Re-estimate
    return history.history['loss'][0]


# ==== TRAINING LOOP ====
def train_rl_agent(model=None, move_to_int=None):
    all_moves = 0
    ...
    for episode in range(TRAINING_EPISODES):
        ...
        while not board.is_game_over():
            reward = collect_experience(board, model, move_to_int, exploration="epsilon")
            loss = None
            training_counter+=1
...
if __name__ == "__main__":
    ...
    train_rl_agent(model=None, move_to_int=move_to_int)
